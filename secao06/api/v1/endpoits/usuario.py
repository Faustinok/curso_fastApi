...
from schemas.usuario_schema import UsuarioSchemaCreate,UsuarioSchemaUp
...
async def put_usuario(usuario_id: int,
    ...
    async with db as session:
        ...
        if usuario_up:
            ...
            if usuario.senha:
                usuario_up = gerar_hash_senha(senha=usuario.senha)
            await session.commit()
        else:
            raise HTTPException(detail ='Usuario nao encontrado',
                                status_code=status.HTTP_404_NOT_FOUND)
...
@router.delete('/{usuario_id}', response_model=UsuarioSchemaBase,
            status_code= status.HTTP_200_OK)
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(),
                db: AsyncSession = Depends(get_session)):
    usuario = await autenticar(email= form_data.username, pwd=form_data.password,db = db)
